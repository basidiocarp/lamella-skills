...
- [Why Multi-Agent Architectures](#why-multi-agent-architectures)
  ...
  - [The Parallelization Argument](#the-parallelization-argument)
  - [The Specialization Argument](#the-specialization-argument)
- [Architectural Patterns](#architectural-patterns)
  - [Pattern 1: Supervisor/Orchestrator](#pattern-1-supervisororchestrator)
  - [Pattern 2: Peer-to-Peer/Swarm](#pattern-2-peer-to-peerswarm)
  - [Pattern 3: Hierarchical](#pattern-3-hierarchical)
- [Context Isolation as Design Principle](#context-isolation-as-design-principle)
  - [Isolation Mechanisms](#isolation-mechanisms)
  - [Isolation Trade-offs](#isolation-trade-offs)
- [Consensus and Coordination](#consensus-and-coordination)
  - [The Voting Problem](#the-voting-problem)
  - [Weighted Contributions](#weighted-contributions)
  - [Debate Protocols](#debate-protocols)
  - [Trigger-Based Intervention](#trigger-based-intervention)
- [Failure Modes and Mitigations](#failure-modes-and-mitigations)
  - [Failure: Supervisor Bottleneck](#failure-supervisor-bottleneck)
  - [Failure: Coordination Overhead](#failure-coordination-overhead)
  - [Failure: Divergence](#failure-divergence)
  - [Failure: Error Propagation](#failure-error-propagation)
- [Applying Patterns in Claude Code](#applying-patterns-in-claude-code)
  - [Command as Supervisor](#command-as-supervisor)
  - [Subagents as Specialists](#subagents-as-specialists)
  - [Files as Shared Memory](#files-as-shared-memory)
  - [Example: Code Review Multi-Agent](#example-code-review-multi-agent)
- [Guidelines](#guidelines)
- [Memory and State Management](#memory-and-state-management)
  - [Working Memory](#working-memory)
  - [Session Memory](#session-memory)
  - [Long-Term Memory](#long-term-memory)
  - [Memory Patterns for Multi-Agent](#memory-patterns-for-multi-agent)
- [Core Concepts](#core-concepts)
- [Detailed Topics](#detailed-topics)
  - [Memory Architecture Fundamentals](#memory-architecture-fundamentals)
  - [Memory Layer Architecture](#memory-layer-architecture)
  - [Memory Implementation Patterns](#memory-implementation-patterns)
  - [Memory Retrieval Patterns](#memory-retrieval-patterns)
...
**Instruction passing:** For simple, well-defined subtasks, the coordinator creates focused instructions. The sub-agent receives only the instructions needed for its specific task. In Claude Code, passing minimal, targeted prompts to subagents via the Task tool.
...
### Competitive Generation (Generate-Critique-Synthesize)

Multiple agents produce competing solutions; independent judges evaluate; adaptive strategy selects outcome:

| Consensus | Strategy | Action |
|-----------|----------|--------|
| All judges agree | SELECT_AND_POLISH | Polish winner with feedback |
| All scores < 3.0 | REDESIGN | Return to generation with lessons |
| Split decision | FULL_SYNTHESIS | Combine best elements |

See [references/competitive/](references/competitive/) for prompt templates and examples.

### Tree of Thoughts (ToT)

Six-phase systematic reasoning: Exploration → Pruning → Expansion → Evaluation → Adaptive Strategy → Synthesis. Use for complex decisions needing multi-path exploration.

See [references/tree-of-thoughts/](references/tree-of-thoughts/) for phase details.

### Swarm Architecture (TeammateTool/Tasks API)

Persistent multi-agent teams with inboxes, task lists, and spawn backends. Leader creates team, spawns teammates, coordinates via messages and shared task queue.

See [references/swarm/](references/swarm/) for teammate operations, spawn backends, and orchestration patterns.
...
| By Component | Modular architectures |
| By Concern | Audits and reviews |
| By File Ownership | Conflict avoidance |

**Dependency graphs:** Minimize chain depth, prefer wide/shallow graphs. Critical path = longest chain = minimum completion time. Never create circular dependencies.

See [references/coordination/](references/coordination/) for decomposition strategies and dependency graphs.

### Parallel Feature Development

**File ownership rule:** One owner per file. When multiple agents need the same file, designate a single owner; others request changes via message.

See [references/parallel-dev/](references/parallel-dev/) for file ownership and merge strategies.
...
Knowledge graphs preserve relationships between entities. Instead of isolated document chunks, graphs encode that Entity A has Relationship R to Entity B. Queries that traverse relationships rather than just similarity.

Temporal knowledge graphs add validity periods to facts. Each fact has a "valid from" and optionally "valid until" timestamp. Time-travel queries that reconstruct knowledge at specific points in time.
...
Queries like "What was the user's address on Date X?" by retrieving facts valid during that date range. It prevents context clash when outdated information contradicts new data. It enables temporal reasoning about how entities changed over time.
...
| File | Path |
|------|------|
| [Examples](references/competitive/examples.md) | `references/competitive/examples.md` |
| [Templates](references/competitive/templates.md) | `references/competitive/templates.md` |
| [Dependency Graphs](references/coordination/dependency-graphs.md) | `references/coordination/dependency-graphs.md` |
| [Task Decomposition](references/coordination/task-decomposition.md) | `references/coordination/task-decomposition.md` |
| [File Ownership](references/parallel-dev/file-ownership.md) | `references/parallel-dev/file-ownership.md` |
| [Merge Strategies](references/parallel-dev/merge-strategies.md) | `references/parallel-dev/merge-strategies.md` |
| [Agent Types](references/swarm/agent-types.md) | `references/swarm/agent-types.md` |
| [Complete Workflows](references/swarm/complete-workflows.md) | `references/swarm/complete-workflows.md` |
| [Error Handling](references/swarm/error-handling.md) | `references/swarm/error-handling.md` |
| [Message Formats](references/swarm/message-formats.md) | `references/swarm/message-formats.md` |
| [Orchestration Patterns](references/swarm/orchestration-patterns.md) | `references/swarm/orchestration-patterns.md` |
| [Spawn Backends](references/swarm/spawn-backends.md) | `references/swarm/spawn-backends.md` |
| [Task System](references/swarm/task-system.md) | `references/swarm/task-system.md` |
| [Teammate Operations](references/swarm/teammate-operations.md) | `references/swarm/teammate-operations.md` |
| [Adaptive Strategies](references/tree-of-thoughts/adaptive-strategies.md) | `references/tree-of-thoughts/adaptive-strategies.md` |
| [Api Design Example](references/tree-of-thoughts/api-design-example.md) | `references/tree-of-thoughts/api-design-example.md` |
| [Phase1 Exploration](references/tree-of-thoughts/phase1-exploration.md) | `references/tree-of-thoughts/phase1-exploration.md` |
| [Phase2 Pruning](references/tree-of-thoughts/phase2-pruning.md) | `references/tree-of-thoughts/phase2-pruning.md` |
| [Phase3 Expansion](references/tree-of-thoughts/phase3-expansion.md) | `references/tree-of-thoughts/phase3-expansion.md` |
| [Phase4 Evaluation](references/tree-of-thoughts/phase4-evaluation.md) | `references/tree-of-thoughts/phase4-evaluation.md` |
| [Phase5 Synthesis](references/tree-of-thoughts/phase5-synthesis.md) | `references/tree-of-thoughts/phase5-synthesis.md` |
