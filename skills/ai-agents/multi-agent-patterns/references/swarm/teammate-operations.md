...
Complete reference for all TeammateTool operations.
...
| Operation | Who Can Use | Purpose |
|-----------|-------------|---------|
| `spawnTeam` | Any agent | Create new team |
| `discoverTeams` | Any agent | List joinable teams |
| `requestJoin` | Non-members | Request team membership |
| `approveJoin` | Leader only | Accept join request |
| `rejectJoin` | Leader only | Decline join request |
| `write` | Team members | Message one teammate |
| `broadcast` | Team members | Message all teammates |
| `requestShutdown` | Leader only | Ask teammate to exit |
| `approveShutdown` | Teammates only | Confirm shutdown |
| `rejectShutdown` | Teammates only | Decline shutdown |
| `approvePlan` | Leader only | Approve plan |
| `rejectPlan` | Leader only | Reject plan with feedback |
| `cleanup` | Leader only | Remove team resources |
