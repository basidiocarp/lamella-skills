...
If a template version exists alongside the regular YAML file, the template version takes precedence. `import: [catalog/file1]` will use `catalog/file1.yaml.tmpl` if it exists, even if `catalog/file1.yaml` also exists.
