#!/usr/bin/env python3
"""Generate a compact onboarding summary for a codebase."""
...
def find_key_configs(root: Path) -> List[str]:
    return [rel for rel in KEY_CONFIG_FILES if (root / rel).exists()]
...
def top_level_structure(root: Path, max_depth: int) -> List[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        visible_files = [name for name in sorted(filenames) if not name.startswith(".")]
...
def build_report(root: Path, max_depth: int) -> Dict[str, object]:
    ...
    extension_counts: Counter[str] = Counter(path.suffix.lower() or "<no-ext>" for path in files)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-depth", type=int, default=2, help="Max depth for structure output")
