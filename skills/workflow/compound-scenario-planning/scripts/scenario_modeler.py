...
class Variable:
    ...
    arr_impact_pct: float       # % of ARR at risk (negative = loss)
...
def calculate_impact(
    ...
    # Select active variables for the requested severity.
    ...
    total_arr_loss_pct = sum(abs(v.arr_impact_pct) for v in active_vars)
    ...
    # Stress and severe cases amplify through domain cascades.
...
def print_report(scenario: Scenario) -> None:
    """Print the full scenario analysis report."""
    print("\n" + "=" * 70)
...
def build_sample_scenario() -> Scenario:
    """Build the bundled compound scenario sample."""
    variables = [
        Variable(
        ...
        Variable(
            ...
            probability=0.25,
            arr_impact_pct=0.0,          # No ARR impact directly.
        ...
        Variable(
            ...
            probability=0.20,
            arr_impact_pct=5.0,          # Roadmap slip causes some revenue impact.
...
def interactive_mode() -> Scenario:
    """Run a simple CLI for building a custom scenario."""
