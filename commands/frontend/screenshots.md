...
### URL/Port Verification (CRITICAL FIRST)

Before capturing any screenshots, verify the application is accessible:

1. **Navigate to Provided URL** — use Playwright to visit the base URL
2. **Verify Correct Application** — check for expected app content (not a default server page or error)
3. **Port Discovery** (if verification fails) — try common ports: 8000, 8080, 3000, 5173, 5174, 5000, 4200
4. **Proceed or Stop** — if correct URL found, update base URL and continue; otherwise STOP and report error
